...
def index():
    ...
    return render_template(
        "index.html", title="Home", surveys=surveys, is_admin=is_admin
    )
...
def survey(survey_id):
    ...
    if form.validate_on_submit():
        question = Question(
            ...
            input_length=form.input_length.data,
            survey=survey,
        )
    ...
    return render_template(
        "survey.html", title=survey.title, survey=survey, form=form, questions=questions
    )
...
def view_results(survey_id):
    ...
    questions = survey.questions.all()
    return render_template(
        "view_results.html", title=survey.title, survey=survey, questions=questions
    )
...
def export_csv(survey_id):
    ...
    for question in questions:
        ...
        for response in responses:
            ...
            cw.writerow([user.username, question.text, response.response_text])
